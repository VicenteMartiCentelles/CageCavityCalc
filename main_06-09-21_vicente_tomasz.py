...
def cavity_volume(positions, weights=None ,radius=1.2, volume_grid_size=0.2):
    # Find min, max for the grid box, and create x,y,z of grid
    x_space = np.arange(np.min(positions.T[0])-radius, np.max(positions.T[0])+radius, volume_grid_size)
    y_space = np.arange(np.min(positions.T[1])-radius, np.max(positions.T[1])+radius, volume_grid_size)
    z_space = np.arange(np.min(positions.T[2])-radius, np.max(positions.T[2])+radius, volume_grid_size)
    ...
    if weights is None:
        ...
        temp_dist = 1*(np.sum(dist_matrix<radius,axis=0)>0)
        temp_weights = weights[np.argmin(dist_matrix,axis=0)]
        hydrophobic_volume = temp_dist.dot(temp_weights)/np.sum(temp_dist)
        return volume, hydrophobic_volume
...
def cavity(frame_index, syst, intput, output, grid_spacing = 1.0, distance_threshold_for_90_deg_angle = 7, save_only_surface = True,calculate_bfactor = True, compute_aromatic_contacts = False, compute_atom_contacts = True, distThreshold_atom_contacts = 5.0, number_of_dummies=500, dummy_atom_diameter=vdw_radii['h'], pore_radius_limit=10):
    ...
    if len(dummies_inside) > 0:
        cageCavityVolume , totalHydrophobicityVolume = cavity_volume(dummy_universe.atoms[dummies_inside].positions, weights=dummy_universe.atoms[dummies_inside].tempfactors, radius=vdwRdummy, volume_grid_size=0.2)
        '''      
        ...
        for i, dummy_atom in enumerate(dummy_universe.atoms[dummies_inside]):
            #There was small mistake here (dummy_universe.atoms[dummies_inside] has diffrent enumeration than dummy_universe.atoms):
            #cavity_hidrophobicity_list.append(dummy_universe.atoms[i].tempfactor)
            #Should be:
            cavity_hidrophobicity_list.append(dummy_atom.tempfactor)
        ...
        '''
        #This can be done without for loop:
        cavity_hidrophobicity_list = dummy_universe.atoms[dummies_inside].tempfactors
        print("Individual hydrophobicity", cavity_hidrophobicity_list)
        print("Total hydrophobicity (summation)", np.mean(cavity_hidrophobicity_list), np.mean(cavity_hidrophobicity_list)*cageCavityVolume)
        print("Total hydrophobicity (integration)", totalHydrophobicityVolume, totalHydrophobicityVolume*cageCavityVolume) 
